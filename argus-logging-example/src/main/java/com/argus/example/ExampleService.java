 ...
 *
 * <h2>Running (dev — logs to console)</h2>
 * <pre>
 * mvn -pl argus-logging-example exec:java -Dexec.mainClass=com.argus.example.ExampleService
 * </pre>
 *
 * <h2>Running (production — OTLP to Datadog agent on localhost:4318)</h2>
 * <pre>
 ...
 * </pre>
 *
 * <h2>Running with transparent tracing (no code changes needed)</h2>
 * <pre>
 * # OTel backend — attach the Argus tracing agent; app code is unchanged
 * java -javaagent:argus-tracing-agent.jar \
 *      -DARGUS_TRACER_BACKEND=otel \
 *      -DOTEL_SERVICE_NAME=example-service \
 *      -jar example.jar
 *
 * # Datadog backend — routes OTLP to the Datadog agent on DD_AGENT_HOST:4318
 * java -javaagent:argus-tracing-agent.jar \
 *      -DARGUS_TRACER_BACKEND=datadog \
 *      -DDD_AGENT_HOST=localhost \
 *      -jar example.jar
 * </pre>
 *
 * <h2>Mandatory OTel envelope (on every record)</h2>
 * <ul>
 *   <li>timestamp — set explicitly at emit() time</li>
 ...
 *   <li>trace_id / span_id — auto-injected when the Argus tracing agent is attached</li>
 * </ul>
 *
...
public class ExampleService {
    ...
    // ArgusLoggerFactory.create() automatically uses GlobalOpenTelemetry when the
    // argus-tracing-agent JAR is attached, giving trace_id/span_id in every log record
    // with zero code changes in this class.
